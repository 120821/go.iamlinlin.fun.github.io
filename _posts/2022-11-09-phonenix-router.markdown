...
<pre>
<code class="makeup shell" translate="no"><span>mix phx.routes</span></code></pre>
...
<pre><code>resources &quot;/posts&quot;, PostController, only: [:index, :show]</code></pre>

<p>查看路由相对的增加：</p>

<pre><code>post_path&nbsp; GET&nbsp;&nbsp;&nbsp;&nbsp; /posts&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; HelloWeb.PostController :index<br />
post_path&nbsp; GET&nbsp;&nbsp;&nbsp;&nbsp; /posts/:id&nbsp; HelloWeb.PostController :show</code></pre>

<p>增加：</p>

<pre><code>resources &quot;/comments&quot;, CommentController, except: [:delete]</code></pre>
...
<pre>
<code class="makeup elixir" translate="no"><span class="nc">HelloWeb.Router.Helpers</span><span class="o">.</span><span class="n">page_path</span><span class="p" data-group-id="4119530099-1">(</span><span class="nc">HelloWeb.Endpoint</span><span class="p">,</span><span class="w"> </span><span class="ss">:index</span><span class="p" data-group-id="4119530099-1">)</span></code></pre>

<pre><code class="makeup elixir" translate="no"><span class="p" data-group-id="4119530099-1">得到结果： &lsquo;/&rsquo;</span></code></pre>

<p><img height="66" src="/uploads/ckeditor/pictures/705/image-20221109170203-2.png" width="960" /></p>

<p>然后，我们就可以使用page_path进行跳转根页面</p>

<pre><code>&lt;%= link &quot;Welcome Page!&quot;, to: Routes.page_path(@conn, :index) %&gt;</code></pre>

<p>当我们为用户资源运行 mix phx.routes 时，它会将 user_path 列为每个输出行的路径辅助函数。&nbsp; 以下是每个动作的含义：</p>

<pre><code>alias HelloWeb.Router.Helpers, as: Routes<br />
...
<pre><code>Routes.user_path(Endpoint, :show, 17, admin: true, active: false)<br />
&quot;/users/17?admin=true&amp;active=false&quot;</code></pre>
...
<pre><code>Routes.user_url(Endpoint, :index)<br />
&quot;http://localhost:4000/users&quot;</code></pre>

<p><img height="77" src="/uploads/ckeditor/pictures/708/image-20221109170916-5.png" width="469" /></p>

<p>使用：</p>

<pre><code>resources &quot;/users&quot;, UserController do<br />
...
<pre><code>alias HelloWeb.Endpoint<br />
HelloWeb.Router.Helpers.user_post_path(Endpoint, :show, 42, 17)<br />
&quot;/users/42/posts/17&quot;</code></pre>
...
review_path&nbsp; DELETE&nbsp; /admin/reviews/:id&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; HelloWeb.Admin.ReviewController :delete</code></pre>

<p>我们得到的实际路线看起来都是正确的，除了每行开头的路径助手 review_path。&nbsp; 对于面向审核路线的用户和管理员路线，我们得到了相同的帮助程序，这是不正确的。</p>

<p>解决：</p>

<pre><code>scope &quot;/admin&quot;, HelloWeb.Admin, as: :admin do<br />
&nbsp; pipe_through :browser
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; PUT&nbsp;&nbsp;&nbsp;&nbsp; /admin/reviews/:id&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; HelloWeb.Admin.ReviewController :update<br />
admin_review_path&nbsp; DELETE&nbsp; /admin/reviews/:id&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; HelloWeb.Admin.ReviewController :delete</code></pre>

<p>进入console ： iex -S mix</p>

<pre><code>HelloWeb.Router.Helpers.review_path(HelloWeb.Endpoint, :index)<br />
&quot;/reviews&quot;

HelloWeb.Router.Helpers.admin_review_path(HelloWeb.Endpoint, :show, 1234)<br />
&quot;/admin/reviews/1234&quot;</code></pre>
...
&nbsp; admin_user_path&nbsp; DELETE&nbsp; /admin/users/:id&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; HelloWeb.Admin.UserController :delete</code></pre>

<p>然后增加：</p>

<pre><code>scope &quot;/api&quot;, HelloWeb.Api, as: :api do<br />
&nbsp; pipe_through :api
