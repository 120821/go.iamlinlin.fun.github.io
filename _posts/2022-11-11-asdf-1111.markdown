...
{% highlight %}然后增加 ~/.bashrc：({% endhighlight %} source ./bashrc){% endhighlight %}
...
<pre class="hljs sql">
 sudo apt-get <span class="hljs-operator"><span class="hljs-keyword">update</span></span>{% endhighlight %}
...
<pre class="hljs sql">
 ...
 sudo docker run -it ubuntu bash</span>{% endhighlight %}

<p><span class="hljs-operator">安装docker compose</span></p>

<p>wget{% highlight %} https://github.com/docker/compose/releases/download/v2.5.0/docker-compose-linux-x86_64(网太差了吧）{% endhighlight %}</p>
...
<p>{% highlight %}实际上：{% endhighlight %}</p>
...
<p>{% highlight %}安装postgres{% endhighlight %}</p>
...
<p><code class="hljs language-ruby"><span class="hljs-symbol">4.安装go caddy</span>{% endhighlight %}</p>

<p>{% highlight %}mkdir /opt/app/go{% endhighlight %}</p>

<p>{% highlight %}进入 /opt/app/go{% endhighlight %}</p>
