...
<p>css调试好，出现了新的报错：</p>
{% highlight html %}Unexpected error while processing request: Missing `secret_token` and `secret_key_base` for &#39;production&#39; environment, set these values in `config/secrets.yml`{% endhighlight %}
...
{% highlight html %}rake assets:precompile --trace RAILS_ENV=production
