...
<blockquote> 
 <p>只更新第一条记录：</p> 
 <pre><code>db.col.update( { "count" : { $gt : 1 } } , { $set : { "test2" : "OK"} } );</code></pre> 
 <p>全部更新：</p> 
 <pre><code>db.col.update( { "count" : { $gt : 3 } } , { $set : { "test2" : "OK"} },false,true );</code></pre> 
 <p>只添加第一条：</p> 
 <pre><code>db.col.update( { "count" : { $gt : 4 } } , { $set : { "test5" : "OK"} },true,false )；</code></pre> 
 <p>全部添加进去:</p> 
