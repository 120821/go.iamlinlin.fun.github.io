...
{% highlight %}<strong>npm install dotenv</strong>
...
<strong>truffle init</strong>{% endhighlight %}

<p><img height="533" src="/uploads/ckeditor/pictures/142/image-20220727100111-1.png" width="919" /></p>

<p>可以看到，truffle 生成了这样几个文件夹：</p>

{% highlight %}$ find contracts/ migrations/ <span class="hljs-built_in">test</span>/<br />
contracts/&nbsp;&nbsp; 智能合约<br />
contracts/Migrations.sol&nbsp;&nbsp; 已经存在了一个文件了<br />
migrations/&nbsp;&nbsp; 迁移<br />
migrations/<span class="hljs-number">1</span>_initial_migration.js&nbsp;&nbsp;&nbsp; 已经存在了一个了<br />
<span class="hljs-built_in">test</span>/&nbsp;&nbsp; 测试文件夹{% endhighlight %}

<p><img height="158" src="/uploads/ckeditor/pictures/143/image-20220727100211-2.png" width="828" /></p>

<p>安装 hdwallet-provider：</p>

{% highlight %}npm install @truffle/hdwallet-provider{% endhighlight %}

<p>创建.env文件</p>

{% highlight %}<span class="hljs-title">(这个URL不能用的)NFURA_API_URL</span> = <span class="hljs-string">&quot;https://goerli.infura.io/v3/427a8f460d514ebfadf0d6c60b0?????&quot;</span>&nbsp;&nbsp; <span class="hljs-comment"># infura的 API URL</span>（如图所示）<br />
MNEMONIC = <span class="hljs-string">&quot;margin blouse tenant chat hub chef addict club rather ??? ??? ??? &quot;</span>&nbsp; <span class="hljs-comment"># metamask 助记词（私钥）</span>{% endhighlight %}
...
{% highlight %}$ cat contracts/Demo.sol<br />
pragma solidity &gt;= <span class="hljs-number">0.5</span><span class="hljs-number">.8</span>;{% endhighlight %}
...
{% highlight %}require(&#39;dotenv&#39;).config();<br />
...
{% highlight %}const Demo_Contract = artifacts.require(&quot;Demo&quot;);{% endhighlight %}

{% highlight %}module.exports = function(deployer) {<br />
...
{% highlight %}&nbsp;&nbsp; &gt; Saving migration to chain.<br />
&nbsp;&nbsp; &gt; Saving artifacts<br />
&nbsp;&nbsp; -------------------------------------<br />
&nbsp;&nbsp; &gt; Total cost:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 0.00084426975 ETH{% endhighlight %}
...
&nbsp;&nbsp; &gt; value sent:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 0 ETH<br />
&nbsp;&nbsp; &gt; total cost:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 0.000391506214187119 ETH{% endhighlight %}
...
&gt; Final cost:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 0.001235775964187119 ETH{% endhighlight %}

{% highlight %}-------ganach（在本地运行）{% endhighlight %}

{% highlight %}ganache<br />
ganache v7.3.2 (@ganache/cli: 0.4.2, @ganache/core: 0.4.2)<br />
Starting RPC server{% endhighlight %}

{% highlight %}Available Accounts<br />
==================<br />
...
(9) 0x0454fDA47462e4b3AFB88cc9699FFeAbD5597B21 (1000 ETH){% endhighlight %}

{% highlight %}Private Keys<br />
==================<br />
...
(9) 0x69fc7d7ccce7ddd1c5c06c62569762e8a4394d12e97bec2dbd42e8e6cefbdbb6{% endhighlight %}

{% highlight %}HD Wallet<br />
==================<br />
Mnemonic:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; tunnel liquid galaxy allow hood crucial upper time describe drip used advance<br />
Base HD Path:&nbsp; m/44&#39;/60&#39;/0&#39;/0/{account_index}{% endhighlight %}

{% highlight %}Default Gas Price<br />
==================<br />
2000000000{% endhighlight %}

{% highlight %}BlockGas Limit<br />
==================<br />
30000000{% endhighlight %}

{% highlight %}Call Gas Limit<br />
==================<br />
50000000{% endhighlight %}

{% highlight %}Chain Id<br />
==================<br />
<strong>1337</strong>{% endhighlight %}

{% highlight %}RPC Listening on <strong>127.0.0.1:8545</strong><br />
eth_blockNumber<br />
net_version<br />
eth_accounts<br />
...
eth_getBlockByNumber<br />
eth_gasPrice<br />
eth_sendTransaction{% endhighlight %}

{% highlight %}&nbsp; Transaction: 0x01a60ca15bf2342459ac622c2f0f0b5c8f7aaecd52a297ed06b01763d445018f<br />
&nbsp; Contract created: 0x64c726875ad7175874cbd5652a708b25595490d8<br />
&nbsp; Gas usage: 250154<br />
&nbsp; Block number: 1<br />
&nbsp; Block time: Wed Jul 27 2022 11:58:18 GMT+0800 (China Standard Time){% endhighlight %}

{% highlight %}eth_getTransactionReceipt<br />
eth_getCode<br />
...
{% highlight %}&nbsp; Transaction: 0x74b55718fa25e224e179ceb090ae03234a2a4e1f1aad1766b1001e069b54c9d5<br />
&nbsp; Gas usage: 45913<br />
&nbsp; Block number: 2<br />
&nbsp; Block time: Wed Jul 27 2022 11:58:18 GMT+0800 (China Standard Time){% endhighlight %}
...
{% highlight %}&nbsp; Transaction: 0xc35d52b0894655f409a9e65dff99ddbea0ce1eb873f3c6ea8cfa86dd09c20799<br />
...
{% highlight %}&nbsp; Transaction: 0xd5cb56a9fadb0830788218e10f46162976fdef32a1dd7fe2d1f12bc89099d8c9<br />
&nbsp; Gas usage: 28813<br />
&nbsp; Block number: 4<br />
&nbsp; Block time: Wed Jul 27 2022 11:58:18 GMT+0800 (China Standard Time){% endhighlight %}
